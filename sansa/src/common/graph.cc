...
void Graph::color_sparsify_fast_d(int c) {
  Timer t;
  t.Start();
...
void Graph::swap_graph_mode() {
  auto tmp_edges = edges;
  auto tmp_vertices = vertices;
  auto tmp_num = n_edges;
...
eidType Graph::reinit_edgelist(bool sym_break, bool ascend) {
  nnz = 0;
  return init_edgelist(sym_break, ascend);
}
