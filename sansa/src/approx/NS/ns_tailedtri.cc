...
void sample_diamond(Graph &g, eidType num_samples, uint64_t &total, uint64_t m) {
  ...
  for (int64_t i = 0; i < num_samples_per_thread; i++) {
    ...
    //v1v2.add(v2);
    //v1v2.add(v1);
    //auto y0 = difference_set(g.N(v0), v1v2);
    auto y0n1 = difference_set(g.N(v0), g.N(v1));
    //if (y0n1.size() < 1) continue;
    auto c1 = difference_num(y0n1, g.N(v2));
    if (c1 < 1) continue;
    counter += double(c0) * c1;
#else
    // for edge-induced case
    auto n = intersection_num(g.N(v0), g.N(v1), v1);
    if (n > 1) counter += n * (d0-2);
#endif
  }
  }
