...
void sample_clique(Graph* g, int k, eidType num_samples, uint64_t &total,  double err_target, double std_devs);
void sample_house(Graph* g, eidType num_samples, uint64_t &total,  double err_target, double std_devs);
void sample_pattern(Graph* g, int k, eidType num_samples, uint64_t &total, Pattern* p,  double err_target, double std_devs);
void sample_path_sb(Graph* g, int k, eidType num_samples, uint64_t &total,  double err_target, double std_devs);


int main(int argc, char* argv[]) {
  if (argc < 5) {
    std::cout << "Usage: " << argv[0] << " <graph> <number_samples_granularity> <pattern_k> <pattern_name> <err_target> <std_devs> <DAG>\n";
  ...
  t.Start();
  uint64_t true_count;
  sample_pattern(&g,k,ns, true_count, &p, e, std);
  t.Stop();
...
void sample_dumbbell(Graph* g, eidType num_samples, uint64_t &total,double err_target, double std_devs) {
  auto m = g->E();
  std::cout << "m = " << m << "\n";
  double counter = 0;
  double counter_sq = 0;
...
void sample_pattern(Graph* g, int k, eidType num_samples, uint64_t &total, Pattern* p,  double err_target, double std_devs) {
  ...
  if(p->is_4clique() || p->is_triangle() || p->is_5clique() || p->is_6clique() || p->is_9clique()) {
    sample_clique(g,k,num_samples,total, err_target, std_devs);
  } else if (p->is_house()) {
    sample_house(g,num_samples, total,  err_target, std_devs);
  } else  if (p->is_triangletriangle()) {
    sample_dumbbell(g,num_samples, total,  err_target, std_devs);
  } else {
    sample_path_sb(g,k,num_samples, total,  err_target, std_devs);
  }
}
