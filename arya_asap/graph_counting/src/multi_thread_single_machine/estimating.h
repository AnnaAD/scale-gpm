...
void tree_DFS_helper(SamplingTreeNode *root, Graph &graph, Pattern &pattern,
                     ...
                     uint32_t *lost_edge_cases, uint64_t *hits) {
    // corner cases
    ...
    for (uint32_t i = 0; i < root->children.size(); i++) {
        tree_DFS_helper(root->children[i], graph, pattern, path, v_map, total_prob_inverse, total_sampling_times, lost_edge_cases, hits);
    }
    path.pop_back();
...
*/
void sampling_tree_count(SamplingTreeNode *root, Graph &graph, Pattern &pattern, double &total_prob_inverse, uint64_t & total_sampling_times, uint32_t *lost_edge_cases, uint64_t *hits) {
    if (FLAG_print)
    ...
    tree_DFS_helper(root, graph, pattern, path, v_map, total_prob_inverse, total_sampling_times, lost_edge_cases, hits);
...
*/
tuple<double, uint64_t> estimate_pattern(Graph &graph, Pattern &pattern, uint64_t sampling_times, uint32_t *lost_edge_cases, default_random_engine &this_rand_generator, uint64_t *hits) {
    ...
    for (uint64_t i = 0; i < sampling_times; i++) {
        ...
        sampling_tree_count(root, graph, pattern, this_prob_inverse, this_num_leaves, lost_edge_cases, hits);
