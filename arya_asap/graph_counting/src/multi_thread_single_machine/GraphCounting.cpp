...
void pattern_counting(Graph &large_graph, Pattern &graph_pattern, uint64_t sampling_times) {
    ...
    tie(this_prob_inverse, this_sampled_times) = estimate_pattern(large_graph, graph_pattern, sampling_times, lost_edge_cases, this_rand_generator, &hits);
...
void multi_thread_pattern_counting(Graph &large_graph, Pattern &graph_pattern, uint64_t sampling_times,
    ...
    if (sampling_times < 1000000)
        sampling_step = 1000;
    double global_prob_inverse = 0.0;  
    
    uint64_t global_hits = 0;                        // global prob inverse
        ...
        workers[t] = make_unique<thread>([&, t]() {
            ...
            while (global_sampled_times < sampling_times) {
                tie(this_prob_inverse, this_sampled_times) = estimate_pattern(large_graph, graph_pattern,
                                                                              sampling_step, lost_edge_cases, this_rand_generator, &hits);
    ...
    cout << "hits: " << global_hits << " out of " << global_sampled_times << " hit rate = " << double(global_hits) / double(global_sampled_times) << "\n";
