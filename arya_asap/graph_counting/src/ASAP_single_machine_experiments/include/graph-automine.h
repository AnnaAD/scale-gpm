...
public:
  GraphA(std::string prefix, bool use_dag = false, bool directed = false,
        bool use_vlabel = false, bool use_elabel = false, 
        bool need_reverse = false, bool bipartite = false);
  GraphA() : name_(""), n_vertices(0), n_edges(0), nnz(0), 
  ...
  GraphA(vidType nv, eidType ne) { allocateFrom(nv, ne); }
  ~GraphA();
  GraphA(const GraphA &)=delete;
  GraphA& operator=(const GraphA &)=delete;
  ...
  void color_sparsify_fast_d(int c);
  ...
  void swap_graph_mode(); 

  vidType *edges_c;               // column indices of CSR format
  eidType *vertices_c;            // row pointers of CSR format
  eidType n_edges_c;              // number of edges
