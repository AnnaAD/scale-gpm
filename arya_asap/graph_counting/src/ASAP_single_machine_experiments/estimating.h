...
tuple<double, double, uint64_t> estimate_pattern_2(Graph &graph, Pattern &pattern, uint64_t sampling_times, uint32_t *lost_edge_cases) {
    //cout << "= Start to estimate patterns, " << sampling_times << " times" << endl;
    double total_prob_inverse = 0.0;
    ...
    for (uint64_t i = 0; i < sampling_times; i++) {
        ...
        this_estimated_pattern_count = this_prob_inverse / this_num_leaves;
        total_prob_inverse += this_estimated_pattern_count;
        total_sum_sq += this_estimated_pattern_count*this_estimated_pattern_count;

    }

    // cout << total_prob_inverse << " " << total_sampling_times << endl;

    return make_tuple(total_prob_inverse, total_sum_sq, sampling_times);
}
...
{   
    ...
    uint64_t /* CHANGED FROM uint32_t HERE */last_edge_idx = graph.ASAP_edge_sampling(rand_generator);
...
{   
    ...
    for (int j = 0; j < 2; j++)
    {
        ...
        total_d_extending_edge = 0;
        vector<uint64_t/* CHANGED FROM uint32_t HERE */> open_set_neighbor;
        for (auto it = open_vertex_set.begin(); it != open_vertex_set.end(); it++)
        {
            // uint32_t begin_idx = max(last_edge_idx + 1, it->second.edge_index);
            uint64_t /* CHANGED FROM uint32_t HERE */begin_idx = it->second.edge_index;
            uint64_t /* CHANGED FROM uint32_t HERE */end_idx = it->second.edge_index + it->second.degree;
            for (uint64_t/* CHANGED FROM uint32_t HERE */ idx = begin_idx; idx < end_idx; idx++) 
            {
                struct Edge edge = graph.edge_list_[idx];
                if (subgraph.vertex_set.find(edge.v_end) != subgraph.vertex_set.end()) 
                    continue;
...
vector<double> ASAP_estimate_4_clique_all(Graph &graph, uint64_t sampling_times, default_random_engine & rand_generator)
{
    vector<double> datapoints(sampling_times);
    long double sum = 0;
    for (uint64_t i = 0; i < sampling_times; i++) 
    {
        datapoints[i] = ASAP_4_clique_neighbor_sampler(graph, rand_generator);
        sum += datapoints[i];
        if ((sampling_times > 1e6) && ((i % 100000) == 0)) cout << "sampled " << i << "," << sum/i << endl;
    }

    return datapoints;
}
tuple<double,double> ASAP_estimate_4_clique_2(Graph &graph, uint64_t sampling_times, default_random_engine & rand_generator)
{
    double total_prob_inverse = 0;
    double total_sum_sq = 0;
    for (uint64_t i = 0; i < sampling_times; i++) 
    {
        double sample = ASAP_4_clique_neighbor_sampler(graph, rand_generator);
        total_prob_inverse += sample;
        total_sum_sq += sample*sample;
    }

    return make_tuple(total_prob_inverse,total_sum_sq);
}
...
vector<double> ASAP_estimate_5_house_all(Graph &graph, uint64_t sampling_times, default_random_engine & rand_generator)
{
    vector<double> datapoints(sampling_times);
    for (uint64_t i = 0; i < sampling_times; i++) 
    {
        datapoints[i] = ASAP_5_house_neighbor_sampler(graph, rand_generator);
        if ((sampling_times > 1e6) && ((i % 100000) == 0)) cout << "sampled " << i << endl;
    }

    return datapoints;
}

tuple<double,double> ASAP_estimate_5_house_2(Graph &graph, uint64_t sampling_times, default_random_engine & rand_generator)
{
    double total_prob_inverse = 0;
    double total_sum_sq = 0;
    for (uint64_t i = 0; i < sampling_times; i++) 
    {
        double sample = ASAP_5_house_neighbor_sampler(graph, rand_generator);
        total_prob_inverse += sample;
        total_sum_sq += sample*sample;
    }
