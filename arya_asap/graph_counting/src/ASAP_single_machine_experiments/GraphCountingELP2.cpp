...
tuple<double, double, uint64_t> multi_thread_pattern_counting(Graph &large_graph, Pattern &graph_pattern, uint64_t sampling_times,
    ...
    for (uint32_t t = 0; t < num_threads; t++) { // Spawn threads
        workers[t] = make_unique<thread>([&, t]() {
            ...
            double this_sum_sq, thread_sum_sq;
            uint64_t thread_sampled_times, this_sampling_times;
            thread_prob_inverse = 0.0;
            thread_sampled_times = 0;

        
            //tie(this_prob_inverse,this_sum_sq,this_sampling_times) = estimate_pattern_2(large_graph, graph_pattern, worker_sampling_times[t],lost_edge_cases/*, this_rand_generator*/);
            tie(this_prob_inverse,this_sum_sq) = ASAP_estimate_4_clique_2(large_graph,worker_sampling_times[t],this_rand_generator);
    ...
    double estimated_variance = global_sum_sq / global_sampled_times - estimated_pattern_count*estimated_pattern_count;
    double estimated_standard_deviation = sqrt(estimated_variance / global_sampled_times);
...
tuple<vector<double>, vector<double> > infer_constant(Graph &sampled_subgraph, Pattern &pattern, uint32_t num_threads, long double &ground_truth)
{
    ...
    uint64_t left_num_estimators = 1e4; // starting from 1e8
    uint64_t this_time_consumed = 0;
    double this_estimated_pattern_count;
    double this_estimated_standard_deviation;
    vector<double> estimated_error;
    vector<double> estimated_ground_truth;
    vector<double> estimated_standard_deviation;

    while (1) {
        cout << "left_num_estimators = " << left_num_estimators << endl;
        tie(this_estimated_pattern_count,this_estimated_standard_deviation,this_time_consumed) \
         = multi_thread_pattern_counting(sampled_subgraph, pattern, left_num_estimators, num_threads);
        cout << "this_estimated_pattern_count = " << this_estimated_pattern_count << endl;
        cout << "this_estimated_standard_deviation = " << this_estimated_standard_deviation << endl;
        estimated_ground_truth.push_back(this_estimated_pattern_count);
        estimated_standard_deviation.push_back(this_estimated_standard_deviation);

        if (this_estimated_pattern_count > 0) {
            // with 5 standard deviation of confidence
            double error = 5*this_estimated_standard_deviation/this_estimated_pattern_count;
    ...
    for (int i = 0; i < estimated_ground_truth.size(); i++) {
        estimated_error.push_back(estimated_standard_deviation[i]/ground_truth);
    }
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    //used_c = max_c;
    //ofstream outfile("data2.txt",std::ios_base::app);
    //outfile << graph_file_path << " " << pattern_file_path << ": K = " << K << endl;
