...
*/
struct Vertex {
    Vertex(uint32_t degree_, uint64_t edge_index_/* CHANGED FROM uint32_t HERE */) : degree(degree_), edge_index(edge_index_) {
    }

    uint32_t degree;
    uint64_t edge_index; /* CHANGED FROM uint32_t HERE */
};
...
public:
    ...
    */
    ...
    void load_binary() {
        GraphA g(graph_file_path_, 0);
        g.print_meta_data();

        g.init_edgelist();

        for(eidType i = 0; i < g.num_edges(); i++) {
            edge_list_.push_back(Edge(g.get_src(i), g.get_dst(i)));
        }

        printf("finished edgelist \n");

        for(vidType v = 0; v < g.num_vertices(); v++) {
            v_table_.emplace(v, Vertex(g.get_degree(v), g.edge_begin(v)));
        }
        ...
        uniform_int_distribution <uint64_t /* CHANGED FROM uint32_t HERE */> new_edge_distribution(0, total_edges_ - 1);
        edge_distribution = new_edge_distribution;

        printf("finished random gen.\n");
    }
    ...
    */
    bool neighbor_test(uint32_t vertex_id_1, uint32_t vertex_id_2) {
        ...
        uint64_t /* CHANGED FROM uint32_t HERE */vertex_1_edge_index = vertex_1.edge_index;
    ...
    */
    ...
    uint64_t/* CHANGED FROM uint32_t HERE */ ASAP_edge_sampling(default_random_engine & rand_generator) {
...
//  private:
    ...
    */
    ...
    uniform_int_distribution <uint64_t/* CHANGED FROM uint32_t HERE */> edge_distribution;
};
